...
export const getCloudImages = async (options = {}) => {
  ...
  const results = await fetch(`https://api.cloudinary.com/v1_1/${process.env.CLOUD_NAME}/resources/image?${paramString}&max_length=100`, {
    headers: {
      Authorization: `Basic ${Buffer.from(process.env.CLOUD_API_KEY + ":" + process.env.CLOUD_API_SECRET).toString("base64")}`
    }
  }).then(res => res.json());
...
export const getFolders = async () => {
  const results = await fetch(`https://api.cloudinary.com/v1_1/${process.env.CLOUD_NAME}/folders`, {
    headers: {
      Authorization: `Basic ${Buffer.from(process.env.CLOUD_API_KEY + ":" + process.env.CLOUD_API_SECRET).toString("base64")}`
    }
  }).then(res => res.json());
