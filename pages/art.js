...
const Art = ({siteConfig, navMenuItems, artItems}) => {
  ...
  return (
    <ArtWrapper>
      ...
      <section>
        {mode !== pageOptions[2] && artItems.images.map((art, index) => (
          <div key={index}>
            <Image src={art.url} alt={art.alt} width="350px" height="350px" />
          </div>
        ))}
        {mode !== pageOptions[1] && artItems.animations.map((video, index) => (
          <iframe src={`https://www.youtube.com/embed/${getYoutubeId(video.url)}?controls=0&autoplay=1&loop=1&rel=0&showinfo=0&mute=1&autohide=1`} key={index} width="350px" height="350px" loop muted frameBorder='0'
            title='video'
            alt={video.alt}
          />
        ))}
